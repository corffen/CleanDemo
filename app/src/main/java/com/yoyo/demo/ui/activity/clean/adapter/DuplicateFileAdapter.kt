...
class DuplicateFileAdapter(private val viewModel: MainViewModel) : ListAdapter<FileItem, RecyclerView.ViewHolder>(FileDiffCallback()) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ITEM_TYPE_HEADER -> {
                val binding = ItemDuplicateFileHeaderBinding.inflate(LayoutInflater.from(parent.context), parent, false)
                HeaderViewHolder(binding)
            }

            ITEM_TYPE_FILE -> {
                val binding = ItemDuplicateFileEntryBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    class HeaderViewHolder(private val binding: ItemDuplicateFileHeaderBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    class FileViewHolder(private val binding: ItemDuplicateFileEntryBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(item: FileItem.FileEntry,viewModel: MainViewModel) {
            ...
            binding.checkBox.setOnCheckedChangeListener { _, isChecked ->
                ...
                viewModel.onFileSelectionChanged(item,isChecked)
            }
        }
    }
}